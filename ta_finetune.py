...
class TaParser:
    ...
    def __process(self):
        try:
            ...
            for index, record in enumerate(self.__data, start=1):
                if (
                    last_record
                    and record.Index - last_record.Index != self.period.value
                ):
    ...
    def __encode_historical_values(self) -> str:
        ...
        try:
            ...
            try:
                next_record_close = self.__complete_df.loc[
                    latest_record + self.period.value, "C"
                ]
            except Exception as _:
                raise ValueError("Next record not available!")
            ...
            next_record_change = self.__scale_non_linear(
                (next_record_close - current_close) / current_close
            )
        ...
        if self.finetune:
            next_val = " -> "
            next_val += f"{self.__encode_hex(next_record_change)}"
...
def run(sequences_dataset, tp):
    ...
    parent_dir = "/home/db/dev/TaSystem/data/finetune/v1"
...
def main(timeperiod: int = 1):
    ...
    for file, tp, _ in files:
        ...
        iteration += 1
        sequences_dataset = []
        print(f"Processing file: {file}, {iteration} out of {len(files)//8}")
